 ...
 */
@Api(description = "demo-api相关接口")
@Slf4j
@RestController
@RequestMapping("/api/demo")
public class DemoApi extends BaseController<DemoPO, DemoQO> {
    ...
    @PostMapping("update")
    public CommonResult update(@Validated({IUpdate.class,Default.class}) @RequestBody DemoPO p) {
        Long tenantId = HttpServletRequestUtil.getTenantId();
        p.setTenantId(tenantId);
        Long userId = HttpServletRequestUtil.getUserId();
        p.setOperatorId(userId);
        return demoService.update(p);
    }


    @ApiOperation(value = "获取详情", httpMethod = "GET", notes = "获取详情")
    @GetMapping("get")
    public CommonResult<DemoPO> get(@RequestParam("id") Long id) {
        DemoQO q = new DemoQO();
        q.setId(id);
        Long tenantId = HttpServletRequestUtil.getTenantId();
        q.setTenantId(tenantId);
    ...
    @GetMapping("list")
    public CommonResult<List<DemoPO>> list(@Validated({IList.class}) DemoQO q) {
        Long tenantId = HttpServletRequestUtil.getTenantId();
        q.setTenantId(tenantId);
    ...
    @GetMapping("listvo")
    public CommonResult<List<DemoVO>> listDemoVO(@Validated({IList.class}) @ModelAttribute("pojo")DemoQO q) {
        Long tenantId = HttpServletRequestUtil.getTenantId();
        q.setTenantId(tenantId);
        return demoService.listDemoVO(q);
    }
}
